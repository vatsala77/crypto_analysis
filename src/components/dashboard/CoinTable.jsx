...
const CoinTable = () => {
  ...
  const { data: coins, isLoading, error, totalPages } = useTopCoins(page, 50);
  const { user } = useAuth();
  ...
  const totalCoins = totalPages * 50;
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-center text-sm text-muted-foreground">
        Showing {(page - 1) * 50 + 1}-{Math.min(page * 50, totalCoins)} of {totalCoins} coins
      </p>
    </div>
  );
};
