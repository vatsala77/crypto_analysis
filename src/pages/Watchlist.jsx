      ...
      <main className="container mx-auto px-4 py-8">
        ...
        >
          ...
          <p className="text-muted-foreground">
            {t('watchlist.savedCoins', { count: watchlist.length })}
          </p>
        </motion.div>
        ...
        {watchlistLoading ? (
          ...
          >
            ...
            <h2 className="text-xl font-semibold mb-2">{t('watchlist.emptyTitle')}</h2>
            <p className="text-muted-foreground mb-6">
              {t('watchlist.emptySubtitle')}
            </p>
            <Button onClick={() => navigate('/')}>
              {t('watchlist.browseCoins')}
            </Button>
          </motion.div>
        ) : (
                ...
                >
                  ...
                  {item.coinData && (
                    <div className="sm:hidden mt-3 pt-3 border-t border-border/50 flex items-center justify-between text-sm">
                      <div>
                        <span className="text-muted-foreground text-xs">{t('dashboard.price')}</span>
                        <p className="font-bold">{formatCurrency(item.coinData.current_price)}</p>
                      </div>
                      <div className="text-right">
                        <span className="text-muted-foreground text-xs">{t('dashboard.change24h')}</span>
                        <p className={cn(
                          'flex items-center gap-0.5 font-medium',
                          item.coinData.price_change_percentage_24h >= 0
                            ? 'text-success'
